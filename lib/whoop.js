...
export function safeWhoopOAuthReason(error) {
  const message = String(error?.message || error || '');
  const providerError = message.match(/^whoop_oauth_\d{3}:([A-Za-z0-9_.-]+)$/)?.[1];
  if (providerError) return `provider_${providerError}`;
  if (/^whoop_(access_denied|invalid_request|invalid_scope|server_error|temporarily_unavailable)$/.test(message)) {
    return message;
  }
  if (['invalid_oauth_state', 'missing_authorization_code', 'whoop_offline_scope_not_granted',
    'whoop_oauth_missing_access_token'].includes(message)) return message;
  if (/^WHOOP_[A-Z_]+_not_configured$/.test(message)) return 'server_not_configured';
  if (/^whoop_api_\d{3}$/.test(message)) return 'profile_fetch_failed';
  return 'unexpected_oauth_error';
}
